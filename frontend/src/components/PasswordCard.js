...
export default function PasswordCard(props) {
  ...
  const Delete = async () => {
    await api
      .delete(
        `${`${process.env.REACT_APP_BACKEND_URL}/password/delete/${props.password.id}/`}`
      )
      .then(() => {
        handleClose();
        window.location.reload();
  ...
  useEffect(() => {
    if (props.password.website) {
      if (props.password.website.startsWith("http"))
        setSrc(`${props.password.website}/favicon.ico`);
      ...
      <Container>
        <Row>
          ...
          <Col>
            ...
            <Row>
              <Col>Username : {props.password.username}</Col>
            </Row>
            <Row>
